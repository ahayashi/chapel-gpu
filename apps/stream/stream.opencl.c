...
extern "C" {
	...
	if (GPUN > 0) {
        ...
        cl_mem c_mem_obj = clCreateBuffer(context, CL_MEM_READ_ONLY, GPUN * sizeof(float), NULL, &ret);
        if (ret != CL_SUCCESS) {
            printf("%s\n", getErrorString(ret));
        }

        cl_event h2d_event[2];
        ret = clEnqueueWriteBuffer(command_queue, b_mem_obj, CL_TRUE, 0, GPUN * sizeof(float), B + start, 0, NULL, &h2d_event[0]);
        if (ret != CL_SUCCESS) {
            printf("%s\n", getErrorString(ret));
        }
        ret = clEnqueueWriteBuffer(command_queue, c_mem_obj, CL_TRUE, 0, GPUN * sizeof(float), C + start, 0, NULL, &h2d_event[1]);
        if (ret != CL_SUCCESS) {
            printf("%s\n", getErrorString(ret));
        }
        ...
        cl_kernel kernel = clCreateKernel(program, "stream", &ret);
        if (ret != CL_SUCCESS) {
            printf("%s\n", getErrorString(ret));
        }
        ...
        ret = clSetKernelArg(kernel, 3, sizeof(cl_float), (void *)&alpha);
        if (ret != CL_SUCCESS) {
            printf("%s\n", getErrorString(ret));
        }

        ret = clSetKernelArg(kernel, 4, sizeof(cl_int), (void *)&GPUN);
        if (ret != CL_SUCCESS) {
            printf("%s\n", getErrorString(ret));
        }
